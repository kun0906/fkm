    ...
    def fit(self, X, record_at=None):
        ...
        for iteration in range(1, 1 + self.max_iter):
            ...
            for i in range(self.n_clusters):
                ...
                if size > 0:
                    update = np.sum(x[mask] - centroids[i],
                                    axis=0)  # get the sum distance (no square distance) between new xs to old centroid i.
...
class KMeansFederated(KMeans):
    ...
    def fit(self, X, record_at=None):
        ...
        for iteration in range(1, 1 + self.max_iter):
            ...
            clients_in_round = r.choice(np.asarray(x, dtype='object'), size=clients_per_round, replace=False)  # without replacement and random
        ...
        for SEED in seeds:  # for repetitions to obtain average and std score.
            ...
            if C == "central":
                ...
                kmeans = KMeansFederated(
                    ...
                    max_iter=ROUNDS,
                    init_centroids='kmeans++',  # hard-code in the function
                    seed=SEED,
                    learning_rate=LR,
                    adaptive_lr=LR_AD,
                    epoch_lr=EPOCH_LR,
