...
from federated_kmeans_kmeans_pp.clustering.gen_dummy import load_federated
from federated_kmeans_kmeans_pp.clustering.my_kmeans import KMeans
from federated_kmeans_kmeans_pp.clustering.utils_func import plot_centroids
from federated_kmeans_kmeans_pp.clustering.utils_func import randomly_init_centroid, record_state, timer
from federated_kmeans_kmeans_pp.clustering.utils_stats import evaluate, plot_stats, plot_progress
...
class KMeansFederated(KMeans):
    ...
    def fit(self, X, record_at=None):
        ...
        for iteration in range(1, 1 + self.max_iter):
            ...
            centroids = centroids + updates  # at the first iteration, the centroids should use the clients' centroids and use the mean of clients' centroids.
...
def run_clustering_federated(verbose=False, dummy=False):
    ...
    for grid_i, C in enumerate(clients_per_round_fractions):
        ...
        history['res'] = res
        plot_centroids(history, title = f'Centralized KMeans with {init_centroids} initialization' if type(C) == str
                                    else f'Federated KMeans with {init_centroids} initialization')
