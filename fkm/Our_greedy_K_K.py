        ...
        for i, client_data in enumerate(clients_in_round):
            if self.use_client_init_centroids:
                ...
                if self.client_init_centroids == 'kmeans++':
                    ...
                    # Calculate seeds from kmeans++
                    client_centroids2, indices2 = kmeans_plusplus(client_data, n_clusters=self.n_clusters,
                                                                random_state=self.random_state*2)
                    client_centroids = np.concatenate([client_centroids1, client_centroids2], axis=0)
