    ...
    def fit(self, X_dict, y_dict, splits=None, record_at=None):
        ...
        for iteration in range(0, self.max_iter):
            ...
            if iteration == 0:
                ...
                elif self.server_init_centroids == 'random':
                    centroids = np.zeros((self.n_clusters, self.dim))
                ...
                elif self.server_init_centroids == 'random_min_max':
                    centroids = np.zeros((self.n_clusters, self.dim))
                    # obtain initial centroids according to clients data (min, max)
                    mn = np.zeros((self.n_clusters, self.dim))
                    mx = np.zeros((self.n_clusters, self.dim))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-S', '--dataset', help='dataset', default='3GAUSSIANS')
    parser.add_argument('-T', '--data_details', help='data details', default='ratio_0.0:mix_clusters_per_client')
    parser.add_argument('-M', '--algorithm', help='algorithm', default='Federated-Server_gaussian')
