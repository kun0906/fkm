...
def compute_client_params(X, centroids):
	...
	# assign to cluster
	labels = np.argmin(sq_dist, axis=1)
	cluster_avg_dists = np.zeros(centroids.shape[0])
	counts =  np.zeros(centroids.shape[0])
	for i in range(centroids.shape[0]):
		mask = np.equal(labels, i)
		counts[i] = np.sum(mask)
		if counts[i] > 0 :
			...
			avg_dist = np.sum(np.square(X[mask] - centroids[i])) / counts[i]   # note here is the square dist
		else:
			avg_dist = 0.0

		cluster_avg_dists[i] = avg_dist

	return centroids, counts, cluster_avg_dists
	...
	def do_federated_round(self, clients_in_round, centroids, iteration):
		...
		for i, client_data in enumerate(clients_in_round):
			...
			if iteration == 0:
				...
				client_centroids, client_counts, client_avg_dists = compute_client_params(client_data,
				                                                                                 client_centroids)
				# client_clusters_sizes == client_counts when iteration > 0
				KM_params['centroids'].extend(list(client_centroids))
				KM_params['cluster_sizes'].extend(list(client_counts))
				# client_avg_dists = updates_sum / np.expand_dims(np.maximum(counts, np.ones_like(counts)), axis=1)
				KM_params['avg_dists'].extend(list(client_avg_dists))  # square distance
	...
	def fit(self, X_dict, y_dict, splits=None, record_at=None):
		...
		for iteration in range(0, self.max_iter):
			...
			if iteration == 0:  # for the first round, the server will select K centroids from K*M client's centroids.
				_, _, KM_params = self.do_federated_round(
				...
				self.initial_centroids, indices = greedily_initialize(KM_centroids, KM_ns, KM_ds, self.n_clusters)
