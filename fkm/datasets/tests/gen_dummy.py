...
from fkm.datasets.adversarial_gaussian3 import adversarial_gaussian3_diff_sigma_n
from fkm.datasets.femnist import femnist_1client_1writer_multidigits, femnist_1client_multiwriters_multidigits, \
    femnist_1client_multiwriters_1digit, femnist_multiusers_per_client
...
from fkm.datasets.nbaiot import nbaiot_user_percent, nbaiot_user_percent_client11
from fkm.datasets.sent140 import sent140_user_percent
...
def load_federated(limit_csv=None, verbose=False, seed=None, clusters=None, n_clients=5, n_clusters=4, rho=2.0,
    ...
    #     return load_femnist_10clients(params, random_state=seed)
        ...
        elif params['p1'] == 'femnist_user_percent':
            return femnist_multiusers_per_client(params, random_state=seed)
    elif params['p0'] == 'NBAIOT':
        if params['p1'] in ['nbaiot_user_percent']:
            return nbaiot_user_percent(params, random_state=seed)
        elif params['p1'] in ['nbaiot_user_percent_client11']:
            return nbaiot_user_percent_client11(params, random_state=seed)
    elif params['p0'] == 'SENT140':
        if params['p1'] == 'sent140_user_percent':
            return sent140_user_percent(params, random_state=seed)
    ...
    elif params['p0'] == '10GAUSSIANS':
        if params['p1'].split(':')[-1] == 'diff_sigma_n':
            return gaussian10_diff_sigma_n(params, random_state=seed)

    elif params['p0'] == '3GAUSSIANS-ADVERSARIAL':
        if params['p1'].split(':')[-1] == 'diff_sigma_n':
            return adversarial_gaussian3_diff_sigma_n(params, random_state=seed)
