...
def server_update(clients_clusters, server_centroids, M, K, D):
    ...
    for k in range(K):
        ...
        for m in range(M):
            ...
            centroids[k] += clients_clusters[m]['centroids'][k] * clients_clusters[m]['sizes'][k]
    ...
    def fit(self, X_dict, y_dict, splits=None, record_at=None):
        ...
        if self.num_clients > 3:
            if  0 < self.sample_fraction < 1:
                n_clients_per_round = max(1, int(self.sample_fraction * self.num_clients))
            else:
        ...
        for iteration in range(0, self.max_iter):
            ...
            # centroids_update2, _  = server_update(clients_clusters, centroids, len(clients_in_round), self.n_clusters, self.dim)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-T', '--data_details', help='data details',
                        default='n1_100-sigma1_0.2_0.2+n2_5000-sigma2_0.2_0.2+n3_10000-sigma3_0.2_0.2:ratio_0.2:diff_sigma_n')
