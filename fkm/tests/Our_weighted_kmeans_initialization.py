...
def compute_weighted_client_params(KM_params, centroids):
	KM_centroids, KM_ns, KM_ds = KM_params['centroids'], KM_params['cluster_sizes'], KM_params['avg_dists']

	mask = np.where(KM_ds == 0)
	KM_ds[mask] = KM_ns[mask]
	weights = (KM_ns / KM_ds).reshape(-1, 1)
	...
	for i in range(centroids.shape[0]):
		...
		ni = np.sum(wi)
		if ni:
			# compute the difference for each point to its centroid
			diff = np.sum(np.multiply(np.repeat(wi, centroids.shape[1], 1), KM_centroids[mask] - centroids[i]), axis=0) / ni
		else:
			diff = 0
		# print(diff, wi, ni)
		new_centroids[i] = centroids[i] + diff
	...
	def fit(self, X_dict, y_dict, splits=None, record_at=None):
		...
		for iteration in range(0, self.max_iter):
			...
			if iteration == 0:  # for the first round, the server will select K centroids from K*M client's centroids.
				KM_centroids, KM_ns, KM_ds = KM_params['centroids'], KM_params['cluster_sizes'], KM_params['avg_dists']
				# 1. select centroids without considering cluster size
				centroids, indices = kmeans_plusplus(KM_centroids, n_clusters=self.n_clusters,
				                                     random_state=self.random_state)
				# weighted kmeans
				for it in range(100):
					new_centroids = compute_weighted_client_params(KM_params, centroids)
					if np.sum(np.square(new_centroids - centroids)) < self.tol:
						break
					centroids = new_centroids
				self.initial_centroids = new_centroids
				print(f'initial_centroids: {self.initial_centroids}, iter: {it}')
