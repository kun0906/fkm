...
import fkm.Centralized_Kmeans
from fkm.cluster.centralized_kmeans import KMeans
from fkm.datasets.gen_dummy import load_federated
from fkm.utils.utils_func import plot_centroids, dump, predict_n_saveimg, \
...
def run_clustering_federated(params, KMeansFederated, verbose=5):
	...
	normalize_method = params['normalize_method']
	splits = params['splits']  # ['train', 'test']
...
def run_model(args):
	...
	args['ALGORITHM']['detail'] = f'R_{N_REPEATS}-M_{N_CLIENTS}-K_{N_CLUSTERS}-{initial_method}-{TOLERANCE}-{NORMALIZE_METHOD}'
	algorithm_detail = args['ALGORITHM']['detail']
	args['OUT_DIR'] = os.path.join(args['OUT_DIR'], dataset_name,  f'{dataset_detail}-SEED_{SEED}.dat',
	                            algorithm_name, algorithm_detail)
	history_file = os.path.join(args['OUT_DIR'], 'history_file.dat')
	if os.path.exists(history_file):
		return history_file

	# settings
	history = {}  # Store all results and needed data
	# fig_paths = []
	server_init_centroids = args['ALGORITHM']['server_init_centroids']
	client_init_centroids = args['ALGORITHM']['client_init_centroids']
	# results = {}
	# for split in splits:
	# 	results[split] = []
	...
	# obtain the true centroids given raw_x and raw_y
	raw_true_centroids = obtain_true_centroids(raw_x, raw_y, SPLITS, args['ALGORITHM'])
	if VERBOSE:
		# print true centroids
		for split in SPLITS:
			true_c = raw_true_centroids[split]
	...
	KMEANS_PY_MAP = {
		'Centralized_KMENAS.py': fkm.cluster.KMeans,
		'federated_server_init_first.py': fkm.Stanford_server_random_initialization.KMeansFederated,
		'federated_client_init_first.py': fkm.Federated_client_initialization.KMeansFederated,
		'federated_greedy_kmeans.py': fkm.Our_greedy_initialization.KMeansFederated
	}
	for idx_seed, seed in enumerate(SEEDS):  # repetitions:  to obtain average and std score.
		...
		kmeans = KMEANS_PY_MAP[args['ALGORITHM']['py_name']](
			n_clusters=N_CLUSTERS,
			# batch_size=BATCH_SIZE,
			sample_fraction=0.5,
		...
		# During the training, we also evaluate the model on the test set at each iteration
		kmeans.fit(X, Y, SPLITS,record_at=False)
		...
		# # save the current 'history' to disk before plotting.
		# seed_file = os.path.join(args['OUR_DIR'], f'SEED_{seed}', f'~history.dat')
		# dump(histories[seed], out_file=seed_file)
		# if algorithm_name != 'FEMNIST':
		# 	try:
		# 		save_history2txt(histories[seed], out_file=seed_file + '.txt')
		# 	except Exception as e:
		# 		print(f'save_history2txt() fails when SEED={seed}, Error: {e}')
		if VERBOSE >= 5:
			pprint(f'seed:{seed}, '
	...
	results_avg = {}
	for split in SPLITS:
		metric_names = history[seed]['scores'][split].keys()
		if split == 'train':
			training_iterations = [vs['training_iterations'] for seed in SEEDS for vs in history[seed]]
			results_avg[split] = {'Iterations': (np.mean(training_iterations), np.std(training_iterations))}
		else:
			results_avg[split] = {'Iterations': ('', '')}
		for k in metric_names:
			value = [vs['scores'][split][k] for seed in SEEDS for vs in history[seed]]
			if k in ['labels_pred', 'labels_true']:
				results_avg[split][k] = value
				continue
			try:
	...
	try:
		title = f'Centralized KMeans with {server_init_centroids} initialization' if not args['IS_FEDERATED'] \
			else f'Federated KMeans with {server_init_centroids} (Server) and {client_init_centroids} (Clients)'
	...
	# out_file = os.path.join( args['OUT_DIR'], f'varied_clients-Server_{server_init_centroids}-Client_{client_init_centroids}')
	# print(out_file)
	# dump(stats, out_file=out_file + '.dat')
	with open(history_file + '.txt', 'w') as file:
		file.write(json.dumps(results_avg))  # use `json.loads` to do the reverse
